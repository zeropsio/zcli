...
func packageStream(ctx context.Context, tempUrl types.String, reader io.Reader) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodPut, tempUrl.Native(), reader)
	if err != nil {
		return errors.Errorf("failed to create upload HTTP request: PUT %s => %v", tempUrl, err)
	}

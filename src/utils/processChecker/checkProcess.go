...
func CheckProcess(ctx context.Context, processId string, apiGrpcClient zeropsApiProtocol.ZeropsApiProtocolClient) error {
	for {
		select {
		case <-ctx.Done():
			return nil
		default:
			getProcessResponse, err := apiGrpcClient.GetProcess(ctx, &zeropsApiProtocol.GetProcessRequest{
				Id: processId,
			})
